  ...
  return (
      ...
      <AppBar position="static" style={{backgroundColor: theme.palette.azul.color}}>
        <Toolbar style={{display:'flex', justifyContent:'space-between'}}>
          <Grid>
            <Button className="navbar-button" style={{ fontSize: isSmallScreen ? '7px' : '16px' }} onClick={() => redirect(route('home'))}>Home</Button>
            <Button className="navbar-button" style={{ fontSize: isSmallScreen ? '7px' : '16px' }} onClick={() => redirect(route('usuarios'))}>Usuarios</Button>
            <Button className="navbar-button" style={{ fontSize: isSmallScreen ? '7px' : '16px' }} onClick={() => redirect(route('grupos'))}>Grupos</Button>
            <Button className="navbar-button" style={{ fontSize: isSmallScreen ? '7px' : '16px' }} onClick={() => redirect(route('estudios'))}>Estudios</Button>
            <Button className="navbar-button" style={{ fontSize: isSmallScreen ? '7px' : '16px' }} onClick={() => redirect(route('alumnos'))}>Alumnos</Button>
            <Button className="navbar-button" style={{ fontSize: isSmallScreen ? '7px' : '16px' }} onClick={() => redirect(route('moverGrupos'))}>Promocionar</Button>
          </Grid>
          <Grid>
